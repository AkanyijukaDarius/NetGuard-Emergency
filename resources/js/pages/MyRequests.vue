<template>
  <f7-page class="bg-gray-50 dark:bg-gray-900">
    ...
    <f7-block class="p-0">
      <f7-pull-to-refresh @refresh="onRefresh">
        <!-- Loading State -->
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <div class="px-4 pb-3 space-y-2">
                ...
                <!-- Location -->
                <div v-if="req.location" class="flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 mt-2">
                  <f7-icon f7="location_fill" size="10"></f7-icon>
                  <span class="truncate">{{ req.location.address || 'Location sent' }}</span>
                </div>
                ...
                <div v-if="req.responder" class="flex items-center gap-3 mt-3 p-2 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                  ...
                  <div class="flex-1 flex flex-wrap items-center gap-2 text-sm">
                    ...
                    <span v-if="req.responder.status" class="text-xs text-green-600 dark:text-green-400">• {{ req.responder.status }}</span>
                  </div>
                </div>

                <!-- Estimated Time -->
                <div v-if="req.status !== 'resolved' && req.status !== 'cancelled'" class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                  <f7-icon f7="hourglass" size="12"></f7-icon>
                  <span>{{ getEstimatedTime(req.status) }}</span>
                </div>
              </div>
